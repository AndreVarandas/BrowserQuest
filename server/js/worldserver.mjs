...
import MobArea from './mobarea.mjs';
import ChestArea from './chestarea.mjs';
import Chest from './chest.mjs';
import Messages from './message.mjs';
import Properties from './properties.mjs';
import Utils from './utils.mjs';
import {Entities, getKindAsString, getKindFromString, isMob, isNpc} from "../../shared/js/gametypes.mjs";
...
class World {
    ...
    removeEntity(entity) {
        ...
        log.debug(
            "Removed " + getKindAsString(entity.kind) + " : " + entity.id,
        )
    }
    ...
    handleHurtEntity(entity, attacker, damage) {
        ...
        if (entity.type === "player") {
            // A player is only aware of his own hit points
    ...
    spawnStaticEntities() {
        ...
        _.each(this.map.staticEntities, function (kindName, tid) {
            var kind = getKindFromString(kindName),
    ...
    getDroppedItem(mob) {
        var kind = getKindAsString(mob.kind),
        ...
        for (var itemName in drops) {
            ...
            if (v <= p) {
                item = this.addItem(
                    this.createItem(getKindFromString(itemName), mob.x, mob.y),
                )
                break
            }
        }
